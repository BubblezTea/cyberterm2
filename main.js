...
async function loadClasses() {
  ...
  function statBar(label, val, colorClass) {
    ...
    const pct = Math.min(100, (displayVal / 100) * 100);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const providerBtns = [
    'provGroq', 'provOllama', 'provOpenAI', 'provOpenrouter',
    'provGemini', 'provQwen', 'provDeepseek', 'provHuggingface'
  ];
  ...
  function openSettings() {
    // Populate current config into input fields
    document.getElementById('cfgGroqKey').value = GROQ_CONFIG.apiKey || '';
    document.getElementById('cfgGroqModel').value = GROQ_CONFIG.model || '';
    document.getElementById('cfgOllamaUrl').value = OLLAMA_CONFIG.url || '';
    document.getElementById('cfgOllamaModel').value = OLLAMA_CONFIG.model || '';
    document.getElementById('cfgOpenAiKey').value = OPENAI_CONFIG.apiKey || '';
    document.getElementById('cfgOpenAiModel').value = OPENAI_CONFIG.model || '';
    document.getElementById('cfgOpenrouterKey').value = OPENROUTER_CONFIG.apiKey || '';
    document.getElementById('cfgOpenrouterModel').value = OPENROUTER_CONFIG.model || '';
    document.getElementById('cfgGeminiKey').value = GEMINI_CONFIG.apiKey || '';
    document.getElementById('cfgGeminiModel').value = GEMINI_CONFIG.model || '';
    document.getElementById('cfgQwenKey').value = QWEN_CONFIG.apiKey || '';
    document.getElementById('cfgQwenModel').value = QWEN_CONFIG.model || '';
    document.getElementById('cfgDeepseekKey').value = DEEPSEEK_CONFIG.apiKey || '';
    document.getElementById('cfgDeepseekModel').value = DEEPSEEK_CONFIG.model || '';
    document.getElementById('cfgHfKey').value = HUGGINGFACE_CONFIG.apiKey || '';
    document.getElementById('cfgHfModel').value = HUGGINGFACE_CONFIG.model || '';
    ...
    // Show/hide fields based on active provider
  ...
  providerBtns.forEach(id => {
    ...
    btn.addEventListener('click', () => {
      ...
      // Update active class on all provider buttons
      providerBtns.forEach(bid => {
        const b = document.getElementById(bid);
        if (b) b.classList.toggle('active', bid === id);
      });

      // Show/hide the appropriate fields container
  ...
  settingsSave.addEventListener('click', () => {
    ...
    GROQ_CONFIG.apiKey = document.getElementById('cfgGroqKey').value.trim();
    GROQ_CONFIG.model = document.getElementById('cfgGroqModel').value.trim() || GROQ_CONFIG.model;
    OLLAMA_CONFIG.url = document.getElementById('cfgOllamaUrl').value.trim() || OLLAMA_CONFIG.url;
    OLLAMA_CONFIG.model = document.getElementById('cfgOllamaModel').value.trim() || OLLAMA_CONFIG.model;
    OPENAI_CONFIG.apiKey = document.getElementById('cfgOpenAiKey').value.trim();
    OPENAI_CONFIG.model = document.getElementById('cfgOpenAiModel').value.trim() || OPENAI_CONFIG.model;
    OPENROUTER_CONFIG.apiKey = document.getElementById('cfgOpenrouterKey').value.trim();
    OPENROUTER_CONFIG.model = document.getElementById('cfgOpenrouterModel').value.trim() || OPENROUTER_CONFIG.model;
    GEMINI_CONFIG.apiKey = document.getElementById('cfgGeminiKey').value.trim();
    GEMINI_CONFIG.model = document.getElementById('cfgGeminiModel').value.trim() || GEMINI_CONFIG.model;
    QWEN_CONFIG.apiKey = document.getElementById('cfgQwenKey').value.trim();
    QWEN_CONFIG.model = document.getElementById('cfgQwenModel').value.trim() || QWEN_CONFIG.model;
    DEEPSEEK_CONFIG.apiKey = document.getElementById('cfgDeepseekKey').value.trim();
    DEEPSEEK_CONFIG.model = document.getElementById('cfgDeepseekModel').value.trim() || DEEPSEEK_CONFIG.model;
    HUGGINGFACE_CONFIG.apiKey = document.getElementById('cfgHfKey').value.trim();
    HUGGINGFACE_CONFIG.model = document.getElementById('cfgHfModel').value.trim() || HUGGINGFACE_CONFIG.model;

    // Validate API key for non-ollama providers
    const needsKey = ['groq', 'openai', 'openrouter', 'gemini', 'qwen', 'deepseek', 'huggingface'];
    if (needsKey.includes(AI_PROVIDER) && !eval(`${AI_PROVIDER.toUpperCase()}_CONFIG.apiKey`)) {
      settingsMsg.textContent = `API KEY REQUIRED FOR ${AI_PROVIDER.toUpperCase()}`;
      settingsMsg.className = 'settings-msg err';
      return;
    }

    saveAiConfig(); // defined in config.js
    settingsMsg.textContent = `SAVED — ${AI_PROVIDER.toUpperCase()} / ${eval(`${AI_PROVIDER.toUpperCase()}_CONFIG.model`)}`;
