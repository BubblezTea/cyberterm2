const Ui = {
  ...
  renderStats() {
    ...
    const dodgePct = Math.min(40, totalAgi * 3);
    const critPct = Math.min(30, Math.floor(totalAgi * 1.5));
    const dmgBonus = Math.floor(totalStr * 0.4);
    ...
    const total = baseVal + equipBonus;         // internal sum (e.g., 5)
    const displayTotal = total * 10;            // display value (e.g., 50)
    const pct = Math.min(100, displayTotal);    // bar width

    const canUp = State.statPoints > 0;

    let display;
    if (overflow > 0 && equipBonus > 0) {
      const totalBonus = overflow + equipBonus;
      display = `${baseVal*10} <span class="rs-bonus">(+${totalBonus*10})</span>`;
    } else if (overflow > 0) {
      display = `${baseVal*10} <span class="rs-bonus">(+${overflow*10})</span>`;
    } else if (equipBonus > 0) {
      display = `${baseVal*10} <span class="rs-bonus gear">(+${equipBonus*10})</span>`;
    } else {
      display = `${baseVal*10}`;
    }
