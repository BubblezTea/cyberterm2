  ...
  renderStats() {
    ...
    const statRows = statDefs.map(d => {
      ...
      if (overflow > 0 && equipBonus > 0) {
        display = `${baseVal} <span class="rs-bonus">(+${overflow + equipBonus})</span>`;
      } else if (overflow > 0) {
        display = `${baseVal} <span class="rs-bonus">(+${overflow})</span>`;
      } else if (equipBonus > 0) {
        display = `${baseVal} <span class="rs-bonus gear">(+${equipBonus})</span>`;
      } else {
        display = `${baseVal}`;
      }
